...
API responses are inconsistent in shape, so **always normalize** through `utils/apiNormalization.js` (e.g. `normalizeEntitiesResponse`, `normalizeMethodsAndConfigs`, `normalizeSynthesisHasLeads`) before using data. Selection-validity guards (`isMethodSelectionValid`, `methodExists`, `deriveConfigId`) live in `utils/contextGuards.js` — predicates only, no cache keys.
