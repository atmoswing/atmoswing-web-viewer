...
AtmoSwing Web Viewer is a React 19 + Vite single-page app that visualizes analog-method precipitation forecasts. It is a **read-only client** for a separate AtmoSwing REST API: it fetches forecast data, renders entities (stations/points) on an OpenLayers map colored by forecast value, and opens modal charts (time series, and forecast details: distributions and analogs). There is no backend in this repo — it builds to static assets served by Nginx.
...
API responses are inconsistent in shape, so **always normalize** before using data. The normalizers live in `utils/normalize/`, one module per API domain (`entities`, `methods`, `analogs`, `series`, `synthesis`, `values`), so an import says which domain it touches. Selection-validity guards (`isMethodSelectionValid`, `methodExists`, `deriveConfigId`) live in `utils/contextGuards.js` — predicates only, no cache keys.

### Caching
`hooks/useCachedRequest.js` is the standard data-fetching hook: `useCachedRequest(key, fetchFn, options)`, backed by a module-level `GLOBAL_CACHE` Map shared across all hook instances, with TTL support and request-cancellation via incrementing request IDs. **The `key` is the single source of truth for refetching** — it must encode every value `fetchFn` reads, since there is no dependency array. A null key (or `enabled: false`) resets the hook to `initialData`, so a cleared selection never leaves the previous selection's data on screen. The returned data always belongs to the current key: on the render where the key changes, the hook answers with the new key's cached entry or `initialData`, never the previous key's result, so a caller validating a choice against a returned list never checks it against another selection's list. Use TTL presets from `utils/cacheTTLs.js` (`SHORT_TTL` 2 min, `DEFAULT_TTL` 5 min, `LONG_TTL` 15 min). `clearCachedRequests(prefix?)` and `invalidateCachedKey(key)` clear entries (workspace switches call `clearCachedRequests()`).

Because the cache is keyed by string, two callers share an entry only when they build the *identical* key. Resources fetched from more than one place therefore get their canonical key and hook in `hooks/forecastQueries.js` — `useMethodsAndConfigs`, `useEntitiesList`, `useReferenceValues`, `useRelevantEntitiesByConfig` — rather than composing a key locally. These take the selection as arguments instead of reading context, so `WorkspaceContext` (which sits outside the forecast providers) can use them too; its methods prefetch and `MethodsContext` now hit one entry. **Never namespace a key by its consumer** (`dist_`, `modal_`): that stores one immutable response under several keys and re-fetches it per consumer. Name keys after the resource. `useRelevantEntitiesByConfig` is the shape to copy when a resource is wanted per item but does not vary per lookup: it loads the relevant entities of every configuration of a method under one key, so the details window's labels and defaults and the time series' configuration resolution share a single request instead of probing configurations one by one.

### Map
`components/map/MapViewer.jsx` orchestrates OpenLayers via focused hooks in `components/map/hooks/` (`useMapInit`, `useForecastPoints`, `useMapInteractions`, `useWorkspaceLayers`, `useOverlayGlobalLayers`, `useProjectionRegistration`, `useDarkMode`). Pure OL helpers (styles, projections, WMTS capabilities, legend stops) live in `components/map/utils/`. Non-WGS84 entity coordinates are reprojected with proj4 — `ENTITIES_SOURCE_EPSG` in config declares the source CRS.
...
### UI structure
`App.jsx` = sidebar + toolbar + map + snackbars, all under an `ErrorBoundary`. Both modals are lazy-loaded (`TimeSeriesModal` from `App.jsx`, `ForecastDetailsModal` from `contexts/ForecastDetailsContext.jsx`) so their D3/chart code stays out of the initial bundle — import them by path, never through a barrel, or the code splitting silently breaks. Each also sits behind its own `LazyModalBoundary` (`modals/common/`), since the root `ErrorBoundary` would otherwise replace the whole app: a modal that fails to render closes with an error snackbar, and a chunk that is missing because the tab predates a deploy (hashed names, `immutable` caching) triggers one guarded page reload — see `utils/staleChunk.js`. The details window is opened through `useForecastDetails().openForecastDetails(selection?)` (`contexts/ForecastDetailsContext.jsx`), whose provider in `App.jsx` owns the window; a given selection (method, config, entity, lead — all optional) overrides the app's current one, and a given config is pinned rather than following the entity's relevance. The two windows hand over to each other, one open at a time: clicking a date in the time series (or its title-bar button, for the date shown on the map) opens the details on that lead, and the details window's time series button goes back for its entity, changing the app's method/config only when the series would otherwise show another forecast. Leads are derived from dates by `leadHours` (`utils/forecastDateUtils.js`) on both sides, and a requested lead that is not offered falls back to the nearest one. Sidebar panels live in `components/panels/`, modals in `components/modals/`; export-to-PDF via jsPDF/svg2pdf, loaded on demand inside `common/chartExport.js` (with the DOM plumbing split out into `common/svgDom.js`). The exporters **reject rather than log**: `ExportMenu` awaits them and raises an error snackbar, so a failed export is visible instead of a menu that just closes. MUI v7 + Emotion for UI; i18n via i18next (`src/i18n.js`, **default language French**, fallback English); translation strings live in `src/locales/en.json` and `fr.json`, one file per language, and a test asserts the two files define identical key sets.

## Testing

Vitest + Testing Library + jsdom. Tests live in `src/__tests__/` mirroring the `src/` tree. `src/__tests__/setup.js` polyfills `matchMedia` and suppresses expected `act()` warnings. Coverage thresholds are enforced in `vitest.config.js` under `coverage.thresholds` (lines 80%, statements/functions 75%, branches 65%) — they must be nested there, since Vitest silently ignores them if set directly on `coverage`. They are calibrated for Vitest 4+'s AST-based V8 remapping, which counts `?.`, `??`, `||` and default parameters as branches, so they read lower than the Vitest 2 numbers of the same tests. No source file is excluded from coverage: the modals' logic lives in hooks that are tested directly, so the components themselves are thin enough to measure. Add tests alongside new utilities/hooks/components following the existing mirrored structure. Vitest 5 rules that bite: `vi.mock`/`vi.hoisted` must sit at the top level of the test file (inside a function they throw), so shared mocks are exported from `src/__tests__/testUtils.js` as module factories and loaded with `vi.mock('react-i18next', async () => (await import('@/__tests__/testUtils.js')).i18nMockModule())`; and a mock the code constructs with `new` must be a `function` or `class`, not an arrow function — watch for code that catches and swallows that error, which leaves a test passing while it tests nothing.
