...
export default function useOverlayConfigLayers({ mapReady, runtimeConfig, workspace, overlayGroupRef, layerSwitcherRef }) {
  useEffect(() => {
    if (!mapReady) return;
    ...
    const polygonStyle = new Style({ stroke: new Stroke({ color: 'rgba(0, 102, 255, 0.9)', width: 2 }), fill: new Fill({ color: 'rgba(0, 102, 255, 0.15)' }) });
    const pointStyle = new Style({ image: new CircleStyle({ radius: 5, stroke: new Stroke({ color: '#003b8e', width: 1.5 }), fill: new Fill({ color: 'rgba(0, 102, 255, 0.7)' }) }) });
    ...
    const addLayerForItem = (item) => {
      ...
      resolveOverlayStyle(item, styleFn).then(sfn => { try { layer.setStyle(sfn); } catch {} });
      ...
      if (lower.endsWith('.geojson') || lower.endsWith('.json')) {
        fetch(url, { cache: 'no-store' })
          ...
          .then(json => {
            ...
            } catch (e) { if (config.API_DEBUG) console.warn('Failed to parse GeoJSON for overlay', title, e); }
          })
        ...
        shp(url)
          .then(geojson => {
            ...
            try {
              ...
              src.addFeatures(feats);
            } catch (e) { if (config.API_DEBUG) console.warn('Failed to parse Shapefile (as GeoJSON) for overlay', title, e); }
          })
