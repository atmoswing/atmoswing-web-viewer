...
export function styleFromConfigObj(obj = {}) {
  ...
  const lineStyle = new Style({
    stroke: new Stroke({
      color: toRGBA(lineStroke.color || obj.strokeColor || '#0066ff'),
      width: Number(lineStroke.width || obj.strokeWidth || 2)
    })
  });
  const polygonStyle = new Style({
    stroke: new Stroke({
      color: toRGBA(polygonStroke.color || obj.strokeColor || '#0066ff'),
      width: Number(polygonStroke.width || obj.strokeWidth || 2)
    }),
  ...
  const pointStyle = new Style({
    image: new CircleStyle({
      ...
      stroke: new Stroke({
        color: toRGBA(pointStroke.color || obj.strokeColor || '#003b8e'),
        width: Number(pointStroke.width || obj.strokeWidth || 1.5)
      }),
...
export async function resolveOverlayStyle(item, defaultsStyleFn) {
  if (item && item.style) {
    try {
      return styleFromConfigObj(item.style);
    } catch (e) {
      void e;
    }
  }
