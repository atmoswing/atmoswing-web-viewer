...
export default function ModalDistributions({open, onClose}) {
  ...
  useEffect(() => {
    setAnalogValues(Array.isArray(analogValuesData) ? analogValuesData : null);
  }, [analogValuesData]);
  ...
  useEffect(() => {
    if (criteriaData) setCriteriaValues(criteriaData.map((v, i) => ({
      index: i + 1,
      value: v
    })).filter(x => x.value != null)); else setCriteriaValues(null);
  }, [criteriaData]);
  ...
  useEffect(() => {
    setReferenceValues(referenceData || null);
  }, [referenceData]);
  ...
  useEffect(() => {
    ...
    if (bestAnalogsData && Array.isArray(bestAnalogsData)) bestAnalogsData.forEach(b => {
        ...
        <Box sx={{display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 2}}>
          <Box sx={{display: 'flex', flexDirection: 'column', gap: 3}}>
            ...
            <Box sx={{display: 'flex', justifyContent: 'center', mt: 1}}>
              <Button variant="outlined" size="small" startIcon={<FileDownloadIcon/>} onClick={openExportMenu}
                      aria-controls={exportAnchorEl ? 'export-menu' : undefined} aria-haspopup="true">
                {t('seriesModal.export')}
              </Button>
              <Menu id="export-menu" anchorEl={exportAnchorEl} open={Boolean(exportAnchorEl)} onClose={closeExportMenu}
